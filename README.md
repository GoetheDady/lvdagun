...
```bash
bun install          # 安装依赖
bun run dev          # 起本地服务(16345)+ vite 开发服务器,自动打开浏览器
...
- 服务默认监听 `127.0.0.1:16345`,可用 `--port` 覆盖
...
- `backend/` — 本地服务(Express + Pi Agent SDK):配置存储、模型目录、SSE 对话流、CLI
- `web/` — 浏览器客户端(React + Vite):配置向导、对话页、设置页

领域术语见 `CONTEXT.md`,架构决策见 `docs/adr/`,产品需求见 `docs/驴打滚-PRD.md`。
