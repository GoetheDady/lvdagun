...
import { CONFIG_FILE, DATA_DIR, FileConfigStore, getOrCreateToken } from './config';
import { createHub } from './hub';
import { createServer } from './server';

/** 默认端口:5 位数、远离常用工具端口,可用 --port 覆盖 */
const DEFAULT_PORT = 16345;
/** dev 模式下 vite 开发服务器的端口(见 web/vite.config.ts) */
...
async function serve(): Promise<void> {
  ...
  const server = app.listen(port, '127.0.0.1', () => {
    ...
    const url = `http://127.0.0.1:${webPort}/?token=${token}`;
