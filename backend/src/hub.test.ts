...
const pi = vi.hoisted(() => ({
  state: {
    ...
    sessionEvents: [] as Array<{ type: string; message?: { role: string }; assistantMessageEvent?: unknown }>,
    abortCalls: 0,
    disposeCalls: 0,
  },
}));
...
vi.mock('@earendil-works/pi-coding-agent', () => {
  ...
  return {
    ModelRuntime: { create: async () => { pi.state.createCount += 1; return runtime; } },
    createAgentSession: async () => {
      ...
      const session = {
        ...
        abort: async () => { pi.state.abortCalls += 1; },
        dispose: () => { pi.state.disposeCalls += 1; },
      };
      return { session };
    },
...
describe('createSession', () => {
  ...
  it('模型不存在时抛错', async () => {
    const hub = createHub({ dataDir: '/tmp/lvdagun-test' });
    await expect(hub.createSession({ provider: 'anthropic', apiKey: '', modelId: 'nope' })).rejects.toThrow();
  });
  ...
  it('prompt 把 Pi 事件流翻译为协议事件,历史只含完整消息', async () => {
    ...
    const hub = createHub({ dataDir: '/tmp/lvdagun-test' });
    const session = await hub.createSession({ provider: 'anthropic', apiKey: '', modelId: 'claude-a' });
    ...
    expect(events[0]).toMatchObject({ type: 'user_message', message: { role: 'user', text: '你好' } });
    // 流式增量逐条转发,delta 顺序保持
