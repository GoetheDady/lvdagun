...
- **`add` 会把作用域 Radix 包装回直接依赖。** CLI 按 registry 声明写入 `@radix-ui/react-<组件>`,但这些包已由统一包 `radix-ui` 传递提供,再声明一遍属冗余 —— 它们曾是本文件的否决方案残留,后经 `bun remove` 清掉。每次 `add` 之后与 `cn` 一并复查 `apps/web/package.json` 的直接依赖。
