...
研究时驴打滚用 `EventSource` 连接会话级 `/events`，服务端保持
`text/event-stream` 响应并逐条写入事件；这与[当时的 ADR](../adr/0004-mirror-pi-json-events-over-sse.md) 选择一致。
这是合理的 V0 设计，但不是上述产品的统一行业模板。此后 ADR-0013 已将正式协议
迁移为一条应用级 WebSocket 承载的 JSON-RPC 2.0。
