...
const assistantMessage = (id: string, text: string): ChatMessage => ({ id, role: 'assistant', text });
...
describe('chatReducer', () => {
  ...
  it('history_loaded 装载历史消息', () => {
    ...
    expect(chatReducer(initialState, { type: 'history_loaded', messages: history }).messages).toEqual(history);
  });

  it('hub_event:user_message 追加到列表', () => {
    const state = chatReducer(initialState, { type: 'hub_event', event: { type: 'user_message', message: userMessage('u1', '你好') } });
  ...
  it('hub_event:流式三连 — start 清空占位、delta 累积、end 定稿', () => {
    ...
    state = chatReducer(state, { type: 'hub_event', event: { type: 'assistant_text_delta', messageId: 'a1', delta: '你' } });
    state = chatReducer(state, { type: 'hub_event', event: { type: 'assistant_text_delta', messageId: 'a1', delta: '好' } });
  ...
  it('hub_event:session_cleared 清空消息并复位流式', () => {
    const before = chatReducer(initialState, { type: 'hub_event', event: { type: 'assistant_message_start', messageId: 'a1' } });
  ...
  it('send_failed 复位发送中并记录可重试错误', () => {
    const state = chatReducer({ ...initialState, sending: true }, { type: 'send_failed', message: '请求失败' });
...
describe('useChatSession', () => {
  it('先拉历史、后订阅事件(事件订阅在历史落地之后)', async () => {
    ...
    vi.mocked(api.getMessages).mockReturnValue(new Promise((resolve) => (resolveHistory = resolve)));
